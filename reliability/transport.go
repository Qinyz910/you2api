...
// GetStreamFirstByteTimeout 首字节超时（用于SSE无响应回退）
func GetStreamFirstByteTimeout() time.Duration {
    return getEnvDuration("STREAM_FIRST_BYTE_TIMEOUT_MS", 8000)
}

// GetTTFBAlertThreshold 首字节耗时预警阈值
func GetTTFBAlertThreshold() time.Duration {
    return getEnvDuration("TTFB_ALERT_THRESHOLD_MS", 2000)
}
